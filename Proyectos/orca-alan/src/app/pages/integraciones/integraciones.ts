...
export class IntegracionesComponent implements OnInit {
  ...
  // Filters
  searchText = signal('');
  filterConnectorType = signal<ConnectorType | null>(null);
  filterStatus = signal<RadioStatus | null>(null);
  filterCategory = signal<string | null>(null);

  // Options for filters
  connectorTypeOptions = [
    { label: 'Todos los tipos', value: null },
    ...CONNECTOR_TYPES.map(c => ({ label: c.label, value: c.value }))
  ];

  statusOptions = [
    { label: 'Todos los estados', value: null },
    ...RADIO_STATUS_OPTIONS.map(s => ({ label: s.label, value: s.value }))
  ];

  categoryOptions = [
    { label: 'Todas las categorias', value: null },
    ...RADIO_CATEGORIES.map(c => ({ label: c.label, value: c.value }))
  ];

  // Computed filtered radios
  filteredRadios = computed(() => {
    return this.integracionesService.filterRadios({
      connectorType: this.filterConnectorType() || undefined,
